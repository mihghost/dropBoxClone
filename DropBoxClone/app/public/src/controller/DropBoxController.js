class DropBoxController {
  constructor() {
    ...
    this.currentFolder = ['hcode'];//atributo para armazenar pastas
    ...
    this.progressBarEl = this.snackModalEl.querySelector(".mc-progress-bar-fg");//procura dentro desse cara a nossa classe
  ...
  connectFirebase() {
        var config = {
            apiKey: "AIzaSyDHPBkCA6JoWPND082wQXGrpm32duG1RqI",
            authDomain: "dropbox-clone-b18f5.firebaseapp.com",
            databaseURL: "https://dropbox-clone-b18f5-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-b18f5",
            storageBucket: "dropbox-clone-b18f5.appspot.com",
            messagingSenderId: "565116229584",
            appId: "1:565116229584:web:6f4e74034693cae2144bd5"
          };
  ...
  getSelection() {
    return this.listFilesEl.querySelectorAll(".selected");//da lista de elementos que temos, retorna todos que estão com a classe selected
  }

  removeTask() {
    let promises = [];

     //usamos a função getSelection para ver quais os arquivos que foram selecionados e um forEach para ir listando esses dados


    this.getSelection().forEach((li) => {
      //precisamos pegar os dados do arquivo que está dentro desse li para transformar em json
    ...
    //uma vez que temos esse arquivo, podemos fazer um push da minha promessa
  ...
  initEvents() {
    ...
    this.btnNewFolder.addEventListener('click', e => {
      ...
      if (name) {
        this.getFirebaseRef().push().set({
          ...
          name, 
          type: 'folder',
          path: this.currentFolder.join('/')//caminho que une cada um dos elementos do array com  o join
        })
      }
    })
    ...
    this.btnDelete.addEventListener("click", (e) => {
      ...
      //como podem ser vários arquivos a serem removidos, usamos uma promiseAll para fazer isso
      this.removeTask()
        .then((responses) => {
    ...
    this.btnRename.addEventListener("click", (e) => {
      let li = this.getSelection()[0];//como o rename só funciona com um seleciondado
      let file = JSON.parse(li.dataset.file);

      let name = prompt("Renomar o arquivo:", file.name);

      if (name) {//verifica se a variável tem algum conteúdo
        file.name = name;

        //altera no firebase
        //vamos procurar um filho que tem a mesma chave, um find em child com a chave dentro do dataset, uma vez que ele encontrou, vamos salvar o objeto que vamos substituir, que é o meu file
    ...
    this.listFilesEl.addEventListener("selectionchange", (e) => {
      ...
      switch (this.getSelection().length) {
        //se não tiver nenhum item selecionado, o nomear e o excluir não devem aparecer
        case 0:
    ...
    this.btnSendFileEl.addEventListener("click", (event) => {
      this.inputFilesEl.click();//seleciona arquivo 
    });
    ...
    this.inputFilesEl.addEventListener("change", (event) => {
      ...
      this.uploadTask(event.target.files)
        .then((responses) => {
          //para cada resposta retornada na promise
  ...
  getFirebaseRef(path) {
    ...
    //se não receber um caminho
    if(!path) path = this.currentFolder.join('/')//join junta os dados do array e coloca o separador que colocamos, neste caso, a barra
...
//método ajax
  ajax(
    ...
    //como o ajax vai acontecer e vai nos dar uma resposta, se deu certo ou não, usamos uma promise para isso
    return new Promise((resolve, reject) => {
      let ajax = new XMLHttpRequest();

      //abrimos nossa conexão via post e mandamos para a pasta upload 
      ajax.open(method, url);


      //como sabemos que o ajax conseguiu quando ele terminou d eenviar o ajax
      ajax.onload = (event) => {
        try {
          //json parse da resposta do servidor
  ...
  //método para receber os arquivos para fazer o upload
  uploadTask(files) {
    let promises = [];//array que irá guardar os arquivos e cada arquivo tem sua promessa

    //precsiamos converter o diles que é uma coleção em um array, fazemos assim:
    [...files].forEach((file) => {
      //para ler um arquivo
      let formData = new FormData();
      //precisamos colocar o arquivos que estamos recebendo dentro do formData
      formData.append("input-file", file);
      //fazemos o push de quantas promessas forem dentro da variável promises
      promises.push(
        this.ajax(
          "/upload",
          "POST",
    ...
    return Promise.all(promises);//verifica todo mundo e dispara cada um dos resolve de cada uma das promises 
  ...
  uploadProgress(event, file) {
    ...
    //para atualizar o css com a porcentagem que a gente tem
  ...
  readFiles() {
    ...
    //pega a referencia dos files.cria um evento de value.uma fot do momento que sera tratada por uma arrow function
    this.getFirebaseRef().on("value", (snapshot) => {
      this.listFilesEl.innerHTML = "";
      //como e uma coleçao de json faz um for each ára cada um dos itens do snapchot
      snapshot.forEach((snapshotItem) => {
        let key = snapshotItem.key;//chave primeira do objeto json no firebase
        let data = snapshotItem.val();//valores desse objeto

        if (data.type){

          this.listFilesEl.appendChild(this.getFileView(data, key));//adiciona os elementos de li 
  ...
  openFolder(){
    ...
    //se já tiver uma pasta aberta, tem-se que parar de ouvi-la para abrir outra
    //dentro de lastFolder devemos desligar o evento que ele tem
  ...
  renderNav(){
    ...
    //salvar o caminho de cada umas das pastas
    let path = [];

    //enquanto o i for menor que o total de pastas que já carreguei
    for(let i = 0; i < this.currentFolder.length; i++){

      //conforme for passando dentro das pastas queremos saber o nome da pasta que tõ agora passando dentro dessse for

      let folderName = this.currentFolder[i];//pasta atual

      let span = document.createElement('span');

      path.push(folderName);

      //para saber se é a última pasta
      ...
      nav.appendChild(span);//appendChild só recebe nó

    }

    this.navEl.innerHTML = nav.innerHTML;//copia o conteúdo do nav que está na memória para o nav que tá na tela
    ...
    this.navEl.querySelectorAll('a').forEach(a =>{
      ...
      a.addEventListener('click', e=>{
        ...
        //saber pra qual pasta ele vai agora
        //recuperamos o dataset e transformamos em array de novo
        this.currentFolder = a.dataset.path.split('/');

        //agora que já sabemos qual é a pasta atual só basta abrirmos ela
